...
par(fig = c(0,1,0,1), las=1, mai=c(0.6,0.0,1.2,0.2))
	...
	for(pop in 1:length(countries_selected_JHU)){
		...
		if(current_pop_insert=="Iran"){
			...
			current_confirmed <- confirmed[current_JHU_country_row,5:ncol(confirmed)]/1000
			current_infected <- output_low95_ifr_china_map_thanatAge_globalPattern_ageSpecificDeaths$total_I[pop,]/1000
			current_infected_low <- output_up95_ifr_china_map_thanatAge_globalPattern_ageSpecificDeaths$total_I[pop,]/1000
...
pdf(file="Figure-3.pdf", width=20, height=10, family="Times", pointsize=24, onefile=TRUE)
	...
	segments(x0=rep(0,3),x1=rep(length(5:ncol(deaths)),3),y0=seq(0.01,0.08,0.01),y1=seq(0.01,0.08,0.01),lty=2,col=grey(0.8))
 	for(pop in c(8,5,3,1)){
		polygon(x=c(c(1:length(5:ncol(deaths))),rev(1:length(5:ncol(deaths)))),y=c(output_up95_ifr_china_map_thanatAge_globalPattern_ageSpecificDeaths$total_lambda[pop,],rev(output_low95_ifr_china_map_thanatAge_globalPattern_ageSpecificDeaths$total_lambda[pop,])),border=NA,col=adjustcolor(pal[pop],alpha.f=0.2))
		lines(x=1:length(5:ncol(deaths)),y=output_mode_ifr_china_map_thanatAge_globalPattern_ageSpecificDeaths$total_lambda[pop,],col=pal[pop],lwd=3)
	}
